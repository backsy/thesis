...
# Connect/create to/a database
con <- dbConnect(RSQLite::SQLite(), 'quadLogs.db')
...
# get connections to all tables
my_db <- src_sqlite(path="quadLogs.db", create = FALSE)
flights <- tbl(my_db, "flight")
timestamps <- tbl(my_db, 'timestamp')
parameters <- tbl(my_db, 'parameter')
messages <- tbl(my_db, 'message')
values <- tbl(my_db, 'value')

# join all tables and drop the ID's
joined_table <- values %>%
  left_join(messages, by = c("message" = "id")) %>%
  left_join(parameters, by = c('parameter' = 'id')) %>%
  left_join(timestamps, by = c('timestamp' = 'id'))# %>%
  #select(id, flight, message.y, timestamp.y, parameter.y, value) #%>%
  #glimpse

#Lets get all current and voltage from each flight
joined_table %>%
  group_by(flight) %>%
  filter(message.x == 5) %>%
  glimpse
